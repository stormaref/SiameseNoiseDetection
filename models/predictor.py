...
class Predictor:
    def __init__(self, train_dataset, test_dataset, transform, num_classes=10, model_type='resnet18', batch_size=256,
        ...
        self.model = Resnet(num_classes=num_classes, model=model_type,
                            pre_trained=pre_trained).to(self.device)

        self.criterion = nn.CrossEntropyLoss()
        self.optimizer = optim.SGD(self.model.parameters(
        ), momentum=0.9, lr=0.01, weight_decay=weight_decay)

    def get_train_val_datasets(self, train_dataset):
        targets = self.get_targets(train_dataset)
        train_idx, val_idx = train_test_split(
            range(len(train_dataset)), test_size=0.2, stratify=targets)
