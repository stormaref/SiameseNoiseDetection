...
class Predictor:
    def __init__(self, train_dataset, test_dataset, transform, num_classes=10, model_type='resnet18', batch_size=256, 
                 device=None, use_validation=False, pre_trained=False, weight_decay=0.001):
        self.use_validation = use_validation
        self.device = device if device else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        test_transform = transforms.Compose([transforms.ToTensor()])
        if use_validation:
            train_dataset, val_dataset = self.get_train_val_datasets(train_dataset)
            self.val_dataset = CustomDataset(val_dataset, test_transform)
            self.val_loader = DataLoader(self.val_dataset, batch_size=batch_size, shuffle=False)
        self.train_dataset = CustomDataset(train_dataset, transform)
        self.test_dataset = CustomDataset(test_dataset, test_transform)
        self.batch_size = batch_size
        
        self.train_loader = DataLoader(self.train_dataset, batch_size=batch_size, shuffle=True)
        self.test_loader = DataLoader(self.test_dataset, batch_size=batch_size, shuffle=False)
        
        self.model = Resnet(num_classes=num_classes, model=model_type, pre_trained=pre_trained).to(self.device)
        
        self.criterion = nn.CrossEntropyLoss()
        self.optimizer = optim.SGD(self.model.parameters(), momentum=0.9, lr=0.01, weight_decay=weight_decay)

    def get_train_val_datasets(self, train_dataset):
        targets = self.get_targets(train_dataset)
        train_idx, val_idx = train_test_split(range(len(train_dataset)), test_size=0.2, stratify=targets)
    ...
    def train(self, num_epochs):
        ...
        self.model.train()
        lr_scheuler = torch.optim.lr_scheduler.StepLR(self.optimizer, 80, 0.1)
