...
class InstanceDependentNoiseAdder(NoiseAdder):
    ...
    def add_noise(self, norm_std=0.1, seed=21):
        ...
        self.noisy_indices = np.where(
            np.array(self.dataset.targets) != np.array(noisy_labels))[0]
    ...
    def get_noisy_labels(self, norm_std, seed):
        ...
        flip_distribution = stats.truncnorm(
            (0 - ratio) / norm_std, (1 - ratio) / norm_std, loc=ratio, scale=norm_std)
        ...
        new_label = [np.random.choice(l, p=P[i])
