"""Analysis and figure generation for :class:`~snd.pipeline.cleaner.NoiseCleaner`.

These methods produce the paper's tables and plots. They are kept apart from the
cleaning pipeline itself and mixed back in, so ``NoiseCleaner`` still exposes them
directly (``cleaner.analyze()``, ``cleaner.calculate_relabeling_score(...)``, ...)
while the orchestration logic in ``cleaner.py`` stays readable.

Everything here reads the per-fold prediction CSVs plus the noise adder's ground
truth; nothing here trains or mutates the cleaned dataset.
"""
import math
import random
