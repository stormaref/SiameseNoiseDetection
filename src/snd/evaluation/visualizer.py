            ...
            for img1, img2, label1, label2, i, j in tqdm(self.dataloader, desc="Extracting Embeddings"):
                ...
                for k in range(label1.size(0)):
                    if label1[k].item() != class1[k].argmax().item():
                        wrong_images.append(
                            (img1[k].cpu(), label1[k].cpu(), class1[k].argmax().cpu(), i))
                    if label2[k].item() != class2[k].argmax().item():
                        wrong_images.append(
                            (img2[k].cpu(), label2[k].cpu(), class2[k].argmax().cpu(), j))
    ...
    def visualize(self, embeddings, real_labels, predicted_labels):
        ...
        for i in range(len(selected_embeddings)):
            for j in range(i + 1, len(selected_embeddings)):
                ...
                print(
                    f"Distance between embedding {random_indices[i]} and {random_indices[j]}: {distance:.4f}")
